...
export function LiquidityPoolPage() {
  ...
  const utilization = pool.totalLiquidity > 0
  ...
  return (
    <div className="animate-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '1.5rem', marginBottom: '1.5rem', alignItems: 'start' }}>
        {/* Pool Ring */}
        <div className="panel" style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
          <h3 style={{ fontSize: '0.9rem', fontWeight: 700 }}>Pool Utilization</h3>
          <div className="pool-ring">
            <svg width="160" height="160" viewBox="0 0 160 160">
              <circle cx="80" cy="80" r={radius} fill="none" stroke="var(--bg-elevated)" strokeWidth="18" />
              <circle cx="80" cy="80" r={radius} fill="none"
                stroke="url(#poolGrad)" strokeWidth="18"
          ...
          <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem' }}>
            <span style={{ color: 'var(--text-muted)' }}>Available</span>
            <span style={{ color: 'var(--accent-green)', fontWeight: 700 }}>{pool.availableLiquidity.toLocaleString()} XLM</span>
          </div>
          <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem' }}>
            <span style={{ color: 'var(--text-muted)' }}>On Loan</span>
            <span style={{ color: 'var(--accent-purple)', fontWeight: 700 }}>{(pool.totalLiquidity - pool.availableLiquidity).toLocaleString()} XLM</span>
          </div>
        </div>
        ...
        <div>
          <div className="stats-grid">
            ...
            <div className="stat-card">
              ...
              <div className="stat-card-trend">across {deposits.length} investors</div>
            </div>
            <div className="stat-card">
              <div className="stat-card-icon">📤</div>
          ...
          {/* My position */}
          {position.shares > 0 && (
            <div style={{ marginTop: '1rem', background: 'linear-gradient(135deg, rgba(0,229,200,0.06), rgba(167,139,250,0.06))', border: '1px solid var(--border-bright)', borderRadius: 'var(--radius-lg)', padding: '1rem 1.25rem' }}>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.5rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>My Position</div>
              <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
                {[
                  { label: 'Shares', value: `${position.shares.toLocaleString()}` },
                  { label: 'Share %', value: `${position.sharePercent.toFixed(1)}%` },
                  { label: 'Value', value: `${position.currentValue.toLocaleString()} XLM` },
                  { label: 'Earned', value: `${position.earnedInterest.toLocaleString()} XLM` },
                ].map(({ label, value }) => (
                  <div key={label}>
                    <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{label}</div>
                    <div style={{ fontWeight: 700, color: 'var(--accent-teal)' }}>{value}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {user?.role !== 'ADMIN' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
          <div className="panel">
            <div className="panel-header"><h3>⬆ Deposit</h3></div>
            <div className="form-field">
              <label className="form-label">Amount (XLM)</label>
              <input className="form-input" type="number" placeholder="1000" value={depositAmt} onChange={e => setDepositAmt(e.target.value)} />
            </div>
            <button className="btn btn-primary btn-full" style={{ marginTop: '1rem' }} onClick={handleDeposit} disabled={loading === 'deposit' || !depositAmt}>
              {loading === 'deposit' ? '⚡ Processing…' : '⬆ Deposit to Pool'}
            </button>
          </div>
          <div className="panel">
            <div className="panel-header"><h3>⬇ Withdraw</h3></div>
            <div className="form-field">
              <label className="form-label">Shares to Redeem</label>
              <input className="form-input" type="number" placeholder="500" value={withdrawAmt} onChange={e => setWithdrawAmt(e.target.value)} />
            </div>
            <button className="btn btn-secondary btn-full" style={{ marginTop: '1rem' }} onClick={handleWithdraw} disabled={loading === 'withdraw' || !withdrawAmt}>
      ...
      <div className="panel">
        ...
        {deposits.map((d, i) => (
          <div key={d.id} className="investor-row">
            <div className="investor-avatar">{d.investorName.split(' ').map(n => n[0]).join('').slice(0, 2)}</div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <div style={{ width: 80 }}>
                  <div className="progress-bar">
                    <div className="progress-fill" style={{ width: `${d.sharePercent}%` }} />
                  </div>
                </div>
