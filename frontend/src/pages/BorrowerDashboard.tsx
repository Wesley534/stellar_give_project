...
import { useFinancing, type FinancingRequest } from '../hooks/useFinancing'
...
export function BorrowerDashboard() {
  ...
  const active = myRequests.filter(r => ['PENDING_ADMIN_REVIEW', 'APPROVED', 'BORROWED'].includes(r.status))
  const history = myRequests.filter(r => ['REPAID', 'REJECTED', 'CLOSED'].includes(r.status))

  const handleSubmit = async () => {
    if (!form.invoiceNumber || !form.invoiceAmount || !form.borrowAmount || !form.repaymentAmount || !form.dueDate) return
    setLoading(true)
  ...
  const totalBorrowed = myRequests.filter(r => r.status === 'BORROWED').reduce((s, r) => s + r.borrowAmount, 0)
  const totalRepaid = myRequests.filter(r => r.status === 'REPAID').reduce((s, r) => s + r.repaymentAmount, 0)
    ...
    <div className="animate-in">
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <div className="stat-card-value">{totalBorrowed.toLocaleString()}</div>
        ...
        <div className="stat-card">
          ...
          <div className="stat-card-value">{history.filter(r => r.status === 'REPAID').length}</div>
          ...
          <div className="animate-in">
            ...
            <div className="form-grid">
              <div className="form-field">
                ...
                <input className="form-input" placeholder="INV-2024-001" value={form.invoiceNumber}
                  onChange={e => setForm(f => ({ ...f, invoiceNumber: e.target.value }))} />
              </div>
              <div className="form-field">
                <label className="form-label">Invoice Amount (XLM)</label>
                <input className="form-input" type="number" placeholder="10000" value={form.invoiceAmount}
                  onChange={e => setForm(f => ({ ...f, invoiceAmount: e.target.value }))} />
              </div>
              <div className="form-field">
                <label className="form-label">Borrow Amount (XLM)</label>
                <input className="form-input" type="number" placeholder="8000" value={form.borrowAmount}
                  onChange={e => setForm(f => ({ ...f, borrowAmount: e.target.value }))} />
              </div>
              <div className="form-field">
                <label className="form-label">Repayment Amount (XLM)</label>
                <input className="form-input" type="number" placeholder="8800" value={form.repaymentAmount}
            ...
            <div className="form-field" style={{ marginTop: '1rem' }}>
              ...
              <textarea className="form-textarea" rows={3} placeholder="Describe the invoice and nature of the financing request…"
                value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
            </div>
            <button className="btn btn-primary" style={{ marginTop: '1.25rem' }} onClick={handleSubmit} disabled={loading}>
      ...
      {active.length > 0 && (
        <div className="panel">
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            {active.map(r => (
              <FinancingRequestCard key={r.id} request={r} role="BORROWER" onBorrow={handleBorrow} onRepay={handleRepay} />
            ))}
          </div>
        </div>
      )}
